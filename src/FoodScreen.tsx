...
const dateKey=(y:number,m:number,d:number)=>[y,String(m+1).padStart(2,"0"),String(d).padStart(2,"0")].join("-");
...
export default function FoodScreen({userId,entries,onChanged}:{userId:string;entries:FoodEntryRow[];onChanged:()=>Promise<void>}){
 ...
 return <ScrollView contentContainerStyle={{padding:20,gap:16,paddingBottom:120}} keyboardShouldPersistTaps="handled">
  ...
  <Card><View style={{flexDirection:"row",alignItems:"center",justifyContent:"space-between"}}><Button secondary label="‹" onPress={()=>setCal(v=>v.m===0?{y:v.y-1,m:11}:{y:v.y,m:v.m-1})}/><Text style={{fontSize:18,fontWeight:"800",color:C.textPrimary}}>{new Date(cal.y,cal.m,1).toLocaleDateString(undefined,{month:"long",year:"numeric"})}</Text><Button secondary label="›" onPress={()=>setCal(v=>v.m===11?{y:v.y+1,m:0}:{y:v.y,m:v.m+1})}/></View>
   <View style={{flexDirection:"row",flexWrap:"wrap"}}>{D.map(d=><Text key={d} style={{width:"14.285%",textAlign:"center",fontSize:11,fontWeight:"700",color:C.textMuted}}>{d[0]}</Text>)}{Array.from({length:new Date(cal.y,cal.m,1).getDay()}).map((_,i)=><View key={"b"+i} style={{width:"14.285%",height:48}}/>)}{Array.from({length:new Date(cal.y,cal.m+1,0).getDate()}).map((_,i)=>{const d=i+1,k=dateKey(cal.y,cal.m,d),count=entries.filter(e=>e.entry_date===k).length,selected=date===k;return <Pressable key={k} onPress={()=>setDate(k)} style={{width:"14.285%",height:48,alignItems:"center",justifyContent:"center",borderRadius:24,borderWidth:selected?2:0,borderColor:C.brandPrimary}}><Text style={{color:C.textPrimary,fontWeight:selected?"800":"500"}}>{d}</Text>{count>0&&<View style={{width:6,height:6,borderRadius:3,marginTop:3,backgroundColor:C.brandSecondary}}/>}</Pressable>})}</View>
   <Text style={{color:C.textMuted}}>Dot = food logged · tap a day to review it or add another entry.</Text>
   <View style={{gap:8,marginTop:6}}>{entries.filter(e=>e.entry_date===date).length===0?<Text style={{color:C.textMuted}}>Nothing logged for this day.</Text>:entries.filter(e=>e.entry_date===date).map(e=><View key={"day-"+e.id} style={{paddingVertical:7,borderBottomWidth:1,borderBottomColor:C.surfaceBorder}}><Text style={{fontWeight:"800",color:C.textPrimary,textTransform:"capitalize"}}>{e.meal_type}</Text><Text style={{color:C.textPrimary}}>{e.description}</Text>{e.tags.length>0&&<Text style={{color:C.textMuted}}>{e.tags.join(" · ")}</Text>}{e.gi_response.length>0&&<Text style={{color:C.textMuted}}>GI: {e.gi_response.join(" · ")}</Text>}</View>)}</View>
  </Card>
